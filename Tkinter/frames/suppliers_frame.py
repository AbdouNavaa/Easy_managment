    ...
    def update_table():
        ...
        for row_index, row in enumerate(data):
            ...
            for col_index, value in enumerate(row):
                ...
                buttons = ctk.CTkButton(data_frame, text="عرض",fg_color="#333",font=ctk.CTkFont(size=12),width=100,
                                        # command=lambda row=row: show_supplier_details(row)
                                        ) 
                buttons.grid(row=row_index, column=0, sticky="nsew", padx=5, pady=5)
...
def create_suppliers_frame(root):
    ...
    show_title_frame(suppliers_frame)
