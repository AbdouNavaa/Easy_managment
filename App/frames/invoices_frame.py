...
def open_update_window(sale, update_callback):
    ...
    canvas = ctk.CTkCanvas(update_window, width=800, height=870)
    ...
    canvas.create_window((0,0), window=frame, anchor="center", width=800,height=870 )
    ...
    default_customer =  f"{sale[2]}---"

    # Créez une variable Tkinter StringVar
    customer = tk.StringVar(value=default_customer)
    ...
    if sale[1]:
        try:
            # Check if sale[1] is already a datetime object
            if isinstance(sale[1], datetime):
                date_entry.set_date(sale[1].date())
            else:
                # If it's a string, parse it
                date_entry.set_date(datetime.strptime(sale[1], '%Y-%m-%d %H:%M:%S').date())
        except ValueError as e:
            messagebox.showerror("Error", "Invalid date format: {sale[1]}, Error: {e}!")
    ...
    status_list = ['في الطريق','مكتمل','ملغى']
    status_default = sale[3]
    status = tk.StringVar(value=status_default)
    ...
    def save_changes():
        customer = customer_entry.get().split('-')[0]
    ...
    ctk.CTkButton(frame, text="حفظ", command=save_changes,font=font_arial_title,corner_radius=2).pack(ipady=10 , fill='x',pady=(5,20),padx=20,)
...
def details_and_update(sale_id,update_callback):
    ...
    # Créer un conteneur scrollable
    canvas = ctk.CTkCanvas(window, width=800, height=700)
    canvas.pack( fill="both", expand=True)
    ...
    # Créer un frame pour contenir les champs du formulaire
    frame = ctk.CTkFrame(canvas, fg_color='#fff',)
    canvas.create_window((0,0), window=frame, anchor="center", width=799, height=700)
